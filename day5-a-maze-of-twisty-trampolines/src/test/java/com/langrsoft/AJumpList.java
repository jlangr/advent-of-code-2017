...
public class AJumpList {
    JumpList jumps;

    @Test
    public void setsCurrentToZeroOnCreate() {
        jumps = createJumps(1);

        assertEquals(0, jumps.currentIndex());
    }

    @Test
    public void incrementsItemAfterJump() {
        jumps = createJumps(1);

        jumps.next();

        assertArrayEquals(new int[]{2}, jumps.updatedList());
    }


    @Test
    public void doesNotUpdateCurrentIfZero() {
        jumps = createJumps(0);
    ...
    @Test
    public void otherwiseUpdatesCurrentOnNext() {
        jumps = createJumps(1, 2);
        jumps.next();
        assertEquals(1, jumps.currentIndex());

        jumps.next();

        assertEquals(3, jumps.currentIndex());
    }

    @Test
    public void supportsNegativeJumps() {
        jumps = createJumps(-1);
    ...
    @Test
    public void jumpsUntilOutOfBounds() {
        jumps = createJumps(0, 3, 0, 1, -3);

        jumps.run();

        assertArrayEquals(new int[] {2, 5, 0, 1, -2}, jumps.updatedList());
    }

    @Test
    public void countsJumps() {
        jumps = createJumps(0, 3, 0, 1, -3);

        jumps.run();

        assertEquals(5, jumps.count());
    }

    @Test
    public void providesNumberOfJumpsForBigAdventTest() {
       jumps = createJumps(
       ...
       assertEquals(339351, jumps.count());
    }

    private JumpList createJumps(int... jumps) {
        return new JumpList(jumps);
    }
}
